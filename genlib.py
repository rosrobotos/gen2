...
def create_creature(gen_length=10, creature_name='one'):
    creature = pd.Series(data=[rnd.randint(0, 1) for i in range(gen_length)], name=creature_name)
    return creature


def create_generation(generation_size=50):
    generation = pd.DataFrame(data=[create_creature(creature_name=(str(i+1)) + 'th') for i in range(generation_size)])
